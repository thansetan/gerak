...
import { ACTIVITY_GROUPS } from '../shared/config';
...
function Dashboard() {
    ...
    if (
        !Object.keys(ACTIVITY_GROUPS)
            .map((g) => g.trim().toLowerCase())
            .includes(initialGroup ?? '')
    ) {
        initialGroup = 'all';
    }
    const loaderData = Route.useLoaderData();
    const [group, setGroup] = useState<string | null>(
        initialGroup?.toLowerCase() ?? null
    );
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-6">
            {isPending ? (
                ...
                <>
                    ...
                    {isFetching && (
                        <p className="font-mono text-xs text-text-muted mb-2 uppercase tracking-tight">
                            Refreshing...
                        </p>
                    )}
                    <ActivityList
