...
import type { ActivitiesResponse, GearDetail } from '../shared/types';
...
export const getGear = createServerFn().handler(async () => {
    const cached =
        await getFromCache<Record<string, GearDetail>>(GEAR_CACHE_KEY);
    if (cached) return cached;

    const activitiesCache =
        await getFromCache<ActivitiesResponse>(ACTIVITIES_CACHE_KEY);
    const gearIds = [
        ...new Set(
    ...
    await setToCache(
        GEAR_CACHE_KEY,
        gearMap,
        getValidatedConfig().gearCacheTTL
    );
    return gearMap;
});
